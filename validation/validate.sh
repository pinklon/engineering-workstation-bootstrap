...
while IFS= read -r file; do bash -n "$file"; done < <(find bin bootstrap -type f -maxdepth 2 | sort)
bash -n install.sh

if command -v shellcheck >/dev/null 2>&1; then
  printf 'Running ShellCheck...\n'
  shellcheck install.sh bin/* bootstrap/*.sh
else
...
if grep -RInE --exclude-dir=.git '(/path/to|<repository-path>|CHANGEME)' .; then
  printf 'FAIL: executable placeholder found.\n' >&2
  exit 1
fi

printf 'Validating dry run...\n'
./bin/workstation-bootstrap dry-run
